module MerchantSamples
  module ApplicationHelper
    def active_li(display, link_path, options = {})
      active_menu = if options.delete(:match_path)
        request.path =~ Regexp.new(link_path)
      else
    ...
    def api_form_for(object, *args, &block)
      ...
      simple_form_for(
        object,
        *(args << options.merge(builder: APIFormBuilder, wrapper: :api_sample_form)),
        &block
      )
    end
    ...
    def get_dotted_hash(hash, prefix = "", values = {})
      plain_objects = hash.select { |k, v| !(v.is_a?(Hash) || v.is_a?(Array)) }
      max_length  = Hash[plain_objects].keys.map(&:size).max
      hash.each do |key, value|
        if value.is_a?(Hash)
          ...
          k = sprintf("%s%-#{max_length}s", prefix, key)
          values[k] = value
        end
      end
      values
